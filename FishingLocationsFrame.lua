...
local function pack(zonemarker, id, collapsible, expanded, level, hour)
    local val;
...
local function unpack(marker)
    ...
    local subzone, id, collapsible, expanded, level, hour;
    local mod = math.fmod;
    hour = mod(val, 100);
    val = floor(val / 100);
    expanded = mod(val, 10);
    ...
    val = floor(val / 10);
    if ( marker < 0 ) then
        return val, 0, collapsible, expanded, level, hour;
    else
        return 0, val, collapsible, expanded, level, hour;
    end
end

local function toggle(marker)
    local z,i,c,e,l,h = unpack(marker);
    e = 1 - e;
    return pack(z,i,c,e,l,h);
end

local function MakeInfo(line, zid, fid, collapsible, level, hour)
    LocationLines[line] = pack(zid, fid, collapsible, collapsible, level, hour);
end
...
local function CountLocationLines()
    ...
    for odx=1,LocationLastLine do
        ...
        if ( check and check ~= 0 ) then
            ...
            local c,e,level;
            _,_,c,e,level,h = unpack(check);
            if ( c == 1 and e == 0 ) then
                local l;
                _,_,_,_,l,h = unpack(LocationLines[j]);
                while ( l > level ) do
                    j = j + 1;
                    _,_,_,_,l,h = unpack(LocationLines[j]);
                end
...
local function FishiesChanged()
    ...
    for i=1,fishcount,1 do
        ...
        MakeInfo(line, 0, fishid, 1, 0, 99);
        line = line + 1;
        FishingBuddy.FishSort(locsort);
        for j=1,table.getn(locsort),1 do
            local exid,exsid,extime = zmex(locsort[j].marker);
            MakeInfo(line, zmto(exid, exsid), 0, 0, 1, 99);
            line = line + 1;
        end
    end
                ...
                for hour=31,34 do
                    ...
                    if ( total > 0 ) then
                        if ( not addedzone ) then
                            MakeInfo(line, zmto(mapId, 0), 0, 1, 0, 99);
                            line = line + 1;
                            addedzone = true;
                        end
                        if ( fh[where] ) then
                            ...
                            MakeInfo(line, where, 0, 1, 2, hour);
                            line = line + 1;
...
local function SubZonesChanged()
    ...
    for i=1,zonecount,1 do
        ...
        if ( ztab ) then
            ...
            if ( oneidx ) then
                MakeInfo(line, oneidx, 0, 1, 0, 99);
                line = line + 1;
                FishingBuddy.FishSort(fishsort);
                for j=1,table.getn(fishsort),1 do
                    local id = fishsort[j].id;
                    MakeInfo(line, 0, id, 0, 1, 99);
                    line = line + 1;
                end
            end
        end
...
FishingBuddy.Locations.Update = function(self, forced)
    ...
    for odx=0,LocationLastLine,1 do
        ...
        if ( info and info ~= 0 ) then
            local zid,fid,c,e,level,hour = unpack(info);
            local percent = nil;
            local zidx, sidx;
            if ( zid > 0 ) then
                lastzid = zid;
                zidx, sidx, _ = zmex(zid);
                if ( sidx > 0 ) then
...
function FishingLocationsCollapseAllButton_OnClick()
    ...
    for j=1,table.getn(LocationLines) do
        local check = LocationLines[j];
        if ( check ~= 0 ) then
            local z,i,c,e,l,h = unpack(check);
            e = Collapsed;
            LocationLines[j] = pack(z,i,c,e,l,h);
