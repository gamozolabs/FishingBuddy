...
-- handle zone markers
local function zmto(zidx, sidx, hour)
    if ( not zidx ) then
        return 0;
    end
    if ( not sidx ) then
    ...
    return zidx*100000 + sidx*100 + hour;
end
FishingBuddy.ZoneMarkerTo = zmto;

local function zmex(packed)
    local sidx = math.fmod(math.floor(packed / 100), 1000);
    local hour = math.fmod(packed, 100);
    return math.floor(packed/100000), sidx, hour;
end
