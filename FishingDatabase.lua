...
local function GetZoneIndex(mapId, subzone, marker, hour)
	initmappings();
	...
	if ( marker ) then
		return zmto(mapId, subzonemapping[mapId][subzone], hour);
	else
...
local function AddFishie(color, id, name, mapId, subzone, texture, quantity, quality, level, it, st, poolhint)
    ...
    -- Totals for all fish for all sub-zones for all hours
    ...
    -- Totals for all fish for a specific sub-zone for all hours
	local timetotidx = zmto(zidx, sidx);
	if( not ft[timetotidx] ) then
		ft[timetotidx] = quantity;
	else
		ft[timetotidx] = ft[timetotidx] + quantity;
	end

    -- Totals for all fish for a specific zone for a specific hour
	local idx = zmto(zidx, sidx, hour);
	if( not ft[idx] ) then
		ft[idx] = quantity;
	else
